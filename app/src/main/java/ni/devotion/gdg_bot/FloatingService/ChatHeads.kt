...
import ni.devotion.gdg_bot.App
import ni.devotion.gdg_bot.ChatFragment
...
class ChatHeads(context: Context) : View.OnTouchListener, FrameLayout(context) {
    ...
    var content = ChatFragment(context)
    ...
    fun collapse() {
        ...
        //OverlayService.instance.stopSelf()
    }
        ...
        if(isClosed){
            ...
            postDelayed({
                ...
                chatHeads.forEach {
                    ...
                    //OverlayService.instance.windowManager.removeViewImmediate(it)
                }
            }, 300)
            //OverlayService.instance.chatHeads.removeAllViews()
//            val serviceInfo = getRunningServiceInfo(OverlayService.instance.javaClass, context)
//            if (serviceInfo != null) {
//                android.os.Process.killProcess(serviceInfo.pid) //Stop the running service
//            }
            OverlayService.instance.unregisterReceiver()
            App.instance.stopService(Intent(App.instance.applicationContext, OverlayService::class.java))
            //content.stopService(Intent(this@ActivityName, ServiceClassName::class.java))
        }else{
            close.hide()
            postDelayed({
                topChatHead!!.springY.currentValue = 0.0
        ...
        when (event!!.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (moving) {
                    ...
                    close.springY.endValue = (metrics.heightPixels - CLOSE_SIZE) + max(((event.rawY + close.height / 2) / 10) - metrics.heightPixels / 10, -WindowManagerHelper.dpToPx(
                        30f
                    ).toFloat()) - WindowManagerHelper.dpToPx(60f).toDouble()
